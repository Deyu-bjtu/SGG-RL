...
class diffusion_model(nn.Module):
    def __init__(self,in_dim,diff_block=None,num_steps=50,out_dims=[128,256,512,256,128],residual=True):
        super().__init__()
        # ********* init parameter *********
        self.num_steps=num_steps
        ...
        self.net=diff_block(in_dim,self.num_steps,out_dims,residual) if diff_block is not None else None
    ...
    def forward(self, x_0, context=None, condition_reps=None,rel_proto=None,rel_nums=None, t=None):
        """
        Args:
        ...
        e_theta = self.net(c0 * x_0 + c1 * e_rand, beta=beta, context=context, condition_reps=condition_reps,rel_proto=rel_proto,t=t,rel_nums=rel_nums)

        return e_theta,e_rand

    def sample_for_training(self,x_0,context=None,condition_reps=None,rel_proto=None,rel_nums=None,rel_labels=None,add_losses=None,sample_steps=10):
        ...
        for t in sample_steps:
            ...
            e_theta = self.net(x_t, beta=beta, context=context, condition_reps=condition_reps, rel_proto=rel_proto, t=[t]*batch_size,rel_nums=rel_nums)
            ...
            if self.training:
                ...
                add_losses['recon_ce_loss']=add_losses.get('recon_ce_loss',0.0)+F.cross_entropy(tail_pre,rel_labels,weight=self.rel_cls_weight.to(x_next.device))
        
        return add_losses

    def sample(self,x_T,context=None,condition_reps=None,rel_proto=None,rel_nums=None,flexibility=0.0, ret_traj=False):
        batch_size = context.size(0)
...
class diffusion_bloack(nn.Module):
    def __init__(self,in_dim,num_steps,out_dims=[128,256,512,256,128],residual=True) -> None:
        ...
        self.enhanced_cond=Trans_block(1,8,128,128,in_dim,in_dim//2)
        self.fused_ctx_cond=MLP(2*in_dim,in_dim,in_dim,2)
    ...
    def forward(self, x, beta, context, condition_reps,rel_proto,t,rel_nums):
        ...
        x=x+time_emb
        condition_reps=self.enhanced_cond(condition_reps,rel_proto.unsqueeze(0).expand(len(rel_nums),-1,-1),rel_nums)
        ctx=self.fused_ctx_cond(torch.cat([context,condition_reps],dim=-1))
...
class VAE(nn.Module):
    def __init__(self,cfg,in_dim,hidden_dim,cluster_num,cof=0.25):
        ...
        self.q_embed=VectorQuantizer(cluster_num,hidden_dim,commitment_cost=cof)
...
class DiffVAE(VAE):
    def __init__(self, cfg, in_dim, hidden_dim, cluster_num,pre_codebook=False, num_steps=50,cof=0.25):
        ...
        self.num_steps=num_steps
        self.diff_module=DiffVAEModel(hidden_dim,num_steps=num_steps,pre_codebook=pre_codebook)
...
class DiffVAEModel(nn.Module):
    def __init__(self,in_dim,num_steps=50,out_dims=[128,256,512,256,128],residual=True,pre_codebook=False):
        ...
        self.net=DiffVAEBlock(in_dim,num_steps,out_dims=self.out_dims,residual=self.residual)
